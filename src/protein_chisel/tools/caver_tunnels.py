...
class CAVERResult:
    ...
    def to_dict(self, prefix: str = "caver__") -> dict[str, float | int]:
        ...
        for i, t in enumerate(self.tunnels[:3]):
            ...
            out[f"{prefix}t{tag}__avg_radius"] = t.avg_radius
        return out
    ...
    with open(pdb_path, "r") as fh:
        for line in fh:
            ...
            if atom is None or atom.record != "HETATM":
                continue
            if atom.res_name != ligand_resname:
                continue
            if atom.element == "H" or atom.name.startswith("H"):
                continue
            coords.append([atom.x, atom.y, atom.z])
    if not coords:
        raise RuntimeError(f"no heavy atoms for ligand {ligand_resname}")
        ...
        for i, row in enumerate(rdr):
            try:
                out.append(Tunnel(
                    ...
                    bottleneck_radius=float(row.get("Bottleneck radius", 0) or 0),
                    curvature=float(row.get("Curvature", 0) or 0),
                    avg_radius=float(row.get("Average R", row.get("Average radius", 0)) or 0),
                ))
